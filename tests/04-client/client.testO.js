...
describe('Client Controller', () => {
    ...
    describe('GET /api/clients/:id', () => {
        it('should retrieve a specific client by ID', async () => {
            console.log(`2Client created ID=${testClient.ID_Client}`)
            const res = await chai.request(app)
                .get(`/api/clients/${testClient.ID_Client}`)
                .set('Authorization', `Bearer ${token}`);
            expect(res).to.have.status(200);
    ...
    describe('PUT /api/clients/:id', () => {
        it('should update a client', async () => {
            const res = await chai.request(app)
                .put(`/api/clients/${testClient.ID_Client}`)
                .set('Authorization', `Bearer ${token}`)
    ...
    describe('DELETE /api/clients/:id', () => {
        it('should delete a client', async () => {
            const res = await chai.request(app)
                .delete(`/api/clients/${testClient.ID_Client}`)
                .set('Authorization', `Bearer ${token}`);
            expect(res).to.have.status(200);
